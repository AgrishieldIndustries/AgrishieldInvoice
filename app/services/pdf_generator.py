...
LOGO_PATH = os.path.join(os.path.dirname(os.path.dirname(__file__)), "assets", "agrishield_logo.png")
...
def generate_invoice_pdf(invoice: Invoice, customer: Customer, company: CompanySettings) -> io.BytesIO:
    ...
    # ── 1. Header: Logo | Company Info | TAX INVOICE block ───────────────────
    # Build logo element (gracefully degrade if file missing)
    logo_cell = []
    if os.path.exists(LOGO_PATH):
        logo_img = Image(LOGO_PATH, width=100, height=58)
        logo_cell = [logo_img]
    else:
        logo_cell = [Paragraph("<b>AGRISHIELD</b>", company_name_style)]

    company_info_cell = [
        Paragraph(company.company_name, company_name_style),
        Spacer(1, 3),
    ...
    # Three-column header: [Logo | Company Details | Invoice Meta]
    header_table = Table(
        [[logo_cell, company_info_cell, tax_invoice_cell]],
        colWidths=[110, 250, 165]
    )
    ...
    story.append(
        Table(
            ...
            colWidths=[525],
            style=[
                ('LINEBELOW', (0, 0), (-1, -1), 2, colors.HexColor('#16A34A')),
    ...
    billing_right = [
        ...
        Paragraph(
            f"<b>State of Supply:</b> {customer.shipping_address.split(',')[-2] if ',' in customer.shipping_address else 'Maharashtra'}",
            meta_val
        ),
        Paragraph(f"<b>Transport Charges:</b> ₹{invoice.transport_charges:,.2f}", meta_val),
        Paragraph(
            f"<b>Taxation Method:</b> {'IGST (Inter-state)' if invoice.igst_total > 0 else 'CGST + SGST (Intra-state)'}",
            meta_val
        ),
    ]
    ...
    for idx, item in enumerate(invoice.items, 1):
        ...
        table_data.append([
            ...
            Paragraph(item.sku.split('-')[-1] if '-' in item.sku else '3105', tb_style),
    ...
    # ── 4. Summary: Bank Details (left) + Totals (right) ──────────────────────
    bank_details_block = [
    ...
    totals_data.append([
        Paragraph("Transport / Loading:", total_label_style),
        Paragraph(f"₹{invoice.transport_charges:,.2f}", total_val_style)
    ])
    totals_data.append([
        Paragraph("Grand Total:", ParagraphStyle('GTL', fontName='Helvetica-Bold', fontSize=10, textColor=colors.HexColor('#16A34A'))),
        Paragraph(f"₹{invoice.grand_total:,.2f}", ParagraphStyle('GTV', fontName='Helvetica-Bold', fontSize=11, textColor=colors.HexColor('#16A34A'), alignment=TA_RIGHT))
    ])
    ...
    story.append(KeepTogether([
        ...
        Table(
            [
                ...
                [
                    Paragraph("Customer Signature", ParagraphStyle('CS', fontName='Helvetica-Bold', fontSize=8, textColor=colors.HexColor('#64748B'))),
                    Paragraph("Authorized Signatory", ParagraphStyle('AS', fontName='Helvetica-Bold', fontSize=8, textColor=colors.HexColor('#64748B'), alignment=TA_RIGHT))
                ]
            ],
            colWidths=[260, 260],
            style=[
